...
#[derive(Debug)]
pub(crate) struct EphemeralKeyBytes(Sec1Point);
...
impl EphemeralKeyBytes {
    fn from_bytes(bytes: [u8; EPK_BYTES]) -> Option<Self> {
        ...
        if encoded.is_compressed()
            && p256::PublicKey::from_sec1_bytes(encoded.as_bytes()).is_ok()
        {
    ...
    pub(crate) fn decompress(&self) -> Sec1Point {
        // EphemeralKeyBytes is a valid compressed encoding by construction.
        let p = p256::PublicKey::from_sec1_bytes(self.0.as_bytes()).unwrap();
...
impl Recipient {
    pub(crate) fn wrap_file_key(&self, file_key: &FileKey) -> RecipientLine {
        let esk = EphemeralSecret::generate_from_rng(&mut UnwrapErr(SysRng));
...
fn salt(epk_bytes: &EphemeralKeyBytes, pk: Sec1Point) -> Vec<u8> {
