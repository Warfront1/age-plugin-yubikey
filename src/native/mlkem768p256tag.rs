...
pub(crate) fn extract_ml_kem_768_seed(cert: &Certificate) -> Option<[u8; 64]> {
    let cert = cert.cert.tbs_certificate();
    let ext = cert
        .extensions()
        .into_iter()
        .flatten()
        .find(|ext| ext.extn_id == ML_KEM_768_SEED_EXTENSION_OID)?;
    let oct = OctetString::from_der(ext.extn_value.as_bytes()).ok()?;
    oct.as_bytes().try_into().ok()
}
